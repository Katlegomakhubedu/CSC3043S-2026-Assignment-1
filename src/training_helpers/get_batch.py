...
def get_batch(token_ids, batch_size, context_length, device, rng=None, fixed_indices=None):
    """
    ...
    params:
        ...
        rng:            a numpy random Generator (used unless fixed_indices is given)
        fixed_indices:  optional array of `batch_size` start positions to use
                         instead of sampling from `rng` - for reproducible
                         validation batches
    returns:
    ...
    if fixed_indices is not None:
        ...
        if rng is None:
            raise ValueError("get_batch needs either `rng` or `fixed_indices`.")
        # Pick random start indices, leaving room for the target shifted one to the right.
        max_start = len(token_ids) - context_length - 1
        starts = rng.integers(0, max_start, size=batch_size)

    # Slice out the inputs and the targets (inputs shifted by one)
    x = np.stack([token_ids[s:s + context_length] for s in starts])
    y = np.stack([token_ids[s + 1:s + 1 + context_length] for s in starts])

    # Convert to int64 tensors on the target device
